...
class CNN(object):
    ...
    def density_matrix(self,sentence_matrix,sentence_matrix_complex,sentence_weighted):
        ...
        input_real_transpose = tf.transpose(self.input_real, perm = [0,1,3,2]) #[batch,seq_len,1,embedding]
        input_imag_transpose = tf.transpose(self.input_img, perm = [0,1,3,2])
        q_a_real_real = tf.matmul(self.input_real,input_real_transpose) #[batch,seq_len,embedding,embedding]
        ...
        '''
        self.input_real = sentence_matrix#[batch,seq_len,embedding]
        self.input_img = sentence_matrix_complex
        input_real_transpose = tf.transpose(self.input_real, perm = [0,2,1])#[batch,embedding,seq_len]
        input_imag_transpose = tf.transpose(self.input_img, perm = [0,2,1])
        q_a_real_real = tf.matmul(input_real_transpose,sentence_weighted)
        q_a_real_real = tf.matmul(q_a_real_real,self.input_real)#[batch,embedding,embedding]
        q_a_real_img = tf.matmul(input_imag_transpose,sentence_weighted)
        q_a_real_img = tf.matmul(q_a_real_img,self.input_img)
        q_a_real = q_a_real_real - q_a_real_img
        q_a_img_real = tf.matmul(input_real_transpose,sentence_weighted)
        q_a_img_real = tf.matmul(q_a_img_real,self.input_img)
        q_a_img_img = tf.matmul(input_imag_transpose,sentence_weighted)
        q_a_img_img = tf.matmul(q_a_img_img,self.input_real)
        q_a_img = q_a_img_real + q_a_img_img
        return q_a_real,q_a_img
